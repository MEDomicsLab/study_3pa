"""
Create experiments for the in-hospital mortality problem using MIMIC-IV and eICU datasets
"""
...
# split mimic dataset
# # Half of Mimic 2008-2013 to train BaseModel and other half to test with the MED3pa method (Internal Validation)
...
for data_name, data_dict in {'Internal': mimic_0813_test,
    ...
    visualize_mdr(result=results, filename=f'experiments/results/in_hospital_mortality/{data_name}/mdr',
                  show=False)
    visualize_tree(result=results, filename=f'experiments/results/in_hospital_mortality/{data_name}/profiles',
                   open_results=False)
...
for hospital_id in eicu_hospitals:
    ...
    if len(eicu_hospital_df) >= 20:
        ...
        visualize_mdr(result=results,
                      filename=f'experiments/results/in_hospital_mortality/External/eicu_hospital_{hospital_id}/mdr',
                      show=False)
        visualize_tree(result=results,
                       filename=f'experiments/results/'
                                f'in_hospital_mortality/External/eicu_hospital_{hospital_id}/profiles',
                       open_results=False)
